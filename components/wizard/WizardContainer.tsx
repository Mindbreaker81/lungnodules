...
const defaultValues = {
  ...
  nodule: {
    ...
    airwayInflammatoryOrInfectious: false,
    atypicalCystCategory: undefined,
    atypicalCystManualOverride: false,
    atypicalCystThickWalled: false,
    atypicalCystMultilocular: false,
    atypicalCystPreviouslyStable: false,
    atypicalCystGrowingCysticComponent: false,
    atypicalCystWallOrCystGrowing: false,
    atypicalCystIncreasedLoculationOrDensity: false,
    atypicalCystAdjacentNodule: false,
    atypicalCystUnilocularThinWalled: false,
    atypicalCystSolidDominant: false,
    isNew: false,
    isSlowGrowing: false,
  },
  ...
  return (
    <Form {...methods}>
      <form className="space-y-6 pb-20" onSubmit={submit}>
        <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
          {steps.map((step) => (
            <div key={step.id} className="flex items-center gap-1">
              <div
                className={`h-2 w-2 rounded-full ${step.id === currentStep ? "bg-primary" : "bg-muted"}`}
                aria-hidden
              />
              <span className={step.id === currentStep ? "font-semibold text-foreground" : ""}>{step.label}</span>
              {step.id !== "results" && <span className="text-muted-foreground/60">/</span>}
            </div>
          ))}
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-10 flex items-center justify-between border-t border-border bg-background/95 backdrop-blur-md px-4 py-3 shadow-md md:hidden">
