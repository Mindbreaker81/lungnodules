...
```
Incidental (Fleischner)  →  Mayo (pre-PET)  +  Herder si hay PET
Screening (Lung-RADS)    →  Brock (pre-PET) +  Herder si hay PET

Modelo recomendado (UI):
  - Herder, si status === "available"
...
2. **Herder logístico (3b):** coeficientes PET **distintos** entre docs:
   - `coefficients.md`: faint +1.439, moderate +3.893, intense +5.534 (sobre log-odds Mayo)
   - `variables_y_coeficientes.md` §3b: faint +2.322, moderate +4.617, intense +4.771 (sobre `P_Mayo` 0–1)  
   - **Bloqueo:** verificar contra Herder 2005; no implementar hasta decidir variante
...
| 2 | (Opcional) **Herder regresión logística** | Herder 2005 Chest | Alternativa o sustitución de LR bayesiano |
...
2. **Herder 3b:** dos juegos de coeficientes PET (ver § Pendientes P0)
...
| **Usuario 2026-06-15:** 74 a, varón, fumador, 7 mm, lóbulo sup., PET intenso | Mayo **~18.7 %** pre-PET; Herder **~69.6 %** post-PET | `incidental 7 mm with intense PET...` |
